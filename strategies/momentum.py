"""
動量策略：RSI + MACD 雙重確認
買入條件：RSI 從超賣回升 + MACD 黃金交叉
賣出條件：RSI 進入超買區 + MACD 死亡交叉
"""
import pandas as pd
import numpy as np
...
class MomentumStrategy(BaseStrategy):
    ...
    def generate_signal(self, code: str, df: pd.DataFrame) -> Optional[Signal]:
        ...
        # 買入訊號：RSI 從超賣回升 + MACD 柱狀圖由負轉正（黃金交叉）
        buy_condition = (
        ...
        # 賣出訊號：RSI 進入超買 + MACD 柱狀圖由正轉負
        sell_condition = (
            rsi_now > self.rsi_overbought
            and hist_now < hist_prev
        )
        ...
        if buy_condition:
            ...
            return Signal(
                ...
                reason=f"RSI={rsi_now:.1f}回升(前={rsi_prev:.1f}) MACD柱增強",
                strategy=self.name,
            )
        ...
        if sell_condition:
            ...
            return Signal(
                ...
                price=price,
                confidence=round(confidence, 2),
                reason=f"RSI={rsi_now:.1f}超買 MACD柱轉弱",
                strategy=self.name,
            )
